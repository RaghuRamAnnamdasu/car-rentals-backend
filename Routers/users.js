...
router.post('/signup', async function (req, res) {
    ...
    const userFromDB = await getUserByName(email);
      ...
      if(userFromDB){
        ...
        if(email === "admin@gmail.com"){
          ...
          const hashedPassword = await genHashedPassword(password);
          const result = await createUSer({email : email , password : hashedPassword, userName: userName, type : "user"});
          console.log(hashedPassword);
          res.send(result);
        }
      }
  ...
  router.post('/login',async function (req, res) {
      ...
      if(!userFromDB){
        ...
        res.send({"message" : "Successful Login", token : token, name: userFromDB.userName, id: userFromDB._id, type: userFromDB.type });
    ...
    console.log(userFromDB);
      ...
      //  const secret = process.env.secretKey + userFromDB.password;
       const secret = process.env.secretKey;
        ...
        console.log(link);
        mailer(email,link);
        const result = await client.db("car-rentals").collection("users").updateOne({email : email},{$set : {resetToken : token}});
        console.log(result);
  ...
  router.post("/resetPassword", async function(req,res){
    ...
    const userDetails = await client.db("car-rentals").collection("users").find({_id : ObjectId(id)}).toArray();
    ...
    jwt.verify(token,process.env.secretKey,async function(err,decodedData){
      if(err){
        ...
        const result = await client.db("car-rentals").collection("users").updateOne({_id : ObjectId(id)},{$set : {password : hashedPassword}});
